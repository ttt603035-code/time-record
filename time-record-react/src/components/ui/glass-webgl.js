"use client";;
import { generateLensMap } from "@/components/ui/glass";
...
class GlassWebGLRenderer {
  ...
  attach(container, lensLayer, source, options) {
    ...
    const tick = () => {
      ...
      this.render(container, lensLayer, source, options, isVideo, supportsRVFC);
  ...
  render(container, lensLayer, source, options, isVideo, supportsRVFC) {
    ...
    gl.uniform1f(this.uniforms.get("uSpecStrength") ?? null, p.specularStrength);
    gl.uniform1f(this.uniforms.get("uSpecularDark") ?? null, p.specularDark ? 1 : 0);
    gl.uniform3f(
    ...
    for (let i = 0; i < SLOTS; i++) {
      ...
      gl.uniform3f(this.uniforms.get("uChromaScale") ?? null, chromaR, chromaG, dispScale);
      gl.uniform2f(this.uniforms.get("uAxis") ?? null, axisX, axisY);
      gl.uniform1f(this.uniforms.get("uTintAmount") ?? null, Math.round(tintEff * 700) / 1000);
